    ...
    public partial class ServerBase : IServerBase, IDisposable
    {
            ...
            List<ServerSecurityPolicy> securityPolicies)
        {
            ...
            for (int ii = 0; ii < baseAddresses.Count; ii++)
            {
                ...
                bool supportsNone = uri.Scheme != Utils.UriSchemeHttps;
                ...
                if (uri.Scheme == Utils.UriSchemeHttps)
                {
                    ...
                    foreach (ServerSecurityPolicy policy in securityPolicies)
                    {
                        if (policy.SecurityPolicyUri == SecurityPolicies.None)
                        {
                            supportsNone = true;
                        }
                    ...
                    description.UserIdentityTokens = GetUserTokenPolicies(configuration, description);
                    description.TransportProfileUri = Profiles.HttpsXmlOrBinaryTransport;
        ...
        /// <summary>
        /// Returns the best URL for the base address based on the URL used by the client.
        /// </summary>
        private string GetBestUrl(Uri clientUrl, BaseAddress baseAddress)
        {
            // nothing to do if no URL provided.
            if (clientUrl == null)
            {
                return baseAddress.Url.ToString();
            }

            UriBuilder builder = new UriBuilder(baseAddress.Url);
            builder.Host = clientUrl.DnsSafeHost;

            // check for variation on localhost name.
            if (NormalizeHostname(clientUrl.DnsSafeHost) == NormalizeHostname("localhost"))
            {
                return builder.ToString();
            }

            // check for match on alternates.
            if (baseAddress.AlternateUrls != null)
            {
                foreach (Uri alternateUrl in baseAddress.AlternateUrls)
                {
                    if (alternateUrl.DnsSafeHost == clientUrl.DnsSafeHost)
                    {
            ...
            // no match on alternates and not a recogonized localhost name.
            // the list was already filtered to only return the same scheme.
            // now use the same DNS name and port (to allow for port forwarding).
            builder.Port = clientUrl.Port;
            return builder.ToString();
        }
        ...
        {
            string url = GetBestUrl(clientUrl, baseAddress);
            ...
            ApplicationDescription application)
        {
            ...
            foreach (EndpointDescription endpoint in endpoints)
            {
                ...
                foreach (BaseAddress baseAddress in baseAddresses)
                {
                    ...
                    translation.EndpointUrl = GetBestUrl(clientUrl, baseAddress);
        ...
        protected class RequestQueue : IDisposable
        {
            ...
            public void ScheduleIncomingRequest(IEndpointIncomingRequest request)
            {
                ...
                lock (m_lock)
                {
                    // check able to scedule requests.
